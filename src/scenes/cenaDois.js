class CenaDois extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("selecao", "src/assets/SelecForn.png")
    ...
    //Carregando imagem do botão da próxima fase
    this.load.image("mecanicaDois", "src/assets/botoes/compButton.png");
    this.load.image('letraE','src/assets/botoes/letraE.png')
    this.load.image('letraE2', 'src/assets/botoes/letraE2.png')
    this.load.image('balaoOlga','src/assets/baloesFala/balao_olga.png')
    this.load.image('olga', 'src/assets/personagens/olga.png')
  ...
  create() {
    ...
    this.player = this.physics.add.sprite(843, 431, "player").setScale(0.2);
    ...
    // adicionando a olga
    this.olga = this.add.image(840, 500, 'olga').setScale(0.21);
    this.physics.world.enable([this.player, this.olga]);
    this.olga.setSize(120, 100);
    this.olga.setDepth(1);

    //adicionando a letra E
    this.letraE4 = this.add.image(875, 512, 'letraE').setScale(0.2);
    this.letraE5 = this.add.image(875, 512, 'letraE2').setScale(0.2);
      
    //Colocando a letra E para piscar
    this.letraE4Interval4 = setInterval(() => {
    this.letraE5.visible = !this.letraE5.visible;
    }, 500);
  
    this.textoFala = this.add.text(765, 512, '', { fontSize: '130%', color: "black", fontFamily :'Verdana', fontWeight: 'extrablack', align: 'justify', wordWrap: { width: 260 }});
    this.textoFala.setVisible(false);
    this.textoFala.setDepth(4);

    // Diálogos de exemplo
    this.dialogos = [
      "Prazer, eu sou a Olga, parabéns por ter superado a 1°zfase do jogo, agora iniciaremos a 2° fase",
      "Prazer, eu sou a Olga, nessa etapa você fará uma análise mais profunda em 3 tópicos para 4 empresas. Estes tópicos são:",
      "Financeiro: Você deve acessar o banco digital para verificar os ganhos, dívidas e os planos de negócios da empresa.",
      "Jurídico: Você deve acessar o jusMeta verificar as questões contratuais e possíveis processos contra as empresas.",
      "Trabalhista: Você deve acessar o reclame ali e verificar o que os ex funcionários da empresa falaram sobre ela",
      "Para isso, dirija-se ao seu computador e verifique os sites de cada empresa",
    ];
    this.dialogoAtual = 0; // Contador para controlar a posição do diálogo atual

    //adicionando balão de fala
    this.balaoOlga = this.add.image(850, 550, 'balaoOlga').setScale(0.4);
    this.balaoOlga.setDepth(3); 
    this.balaoOlga.setVisible(false);
    ...
    this.physics.add.overlap(this.player, this.olga, () => {
      this.playerOverlappingOlga = true;
    }, null, this);

    this.input.keyboard.on('keydown-E', () => {
      if (this.playerOverlappingOlga) {
        // Lógica para exibir o diálogo entre o jogador e o NPC
        this.balaoOlga.setVisible(true);
        ...
        if (this.dialogoAtual > this.dialogos.length) {
          ...
          this.balaoOlga.setVisible(false)
          this.textoFala.setVisible(false)
          this.olga.setVisible(false)
          this.letraE4.setVisible(false)
          this.letraE5.setVisible(false)
          clearInterval(this.letraE4Interval4);
      ...
      this.letraE20 = this.add.image(677, 412, 'letraE').setScale(0.2);
      this.letraE21 = this.add.image(677, 412, 'letraE2').setScale(0.2);
      this.letraE20.setVisible(false)
      this.letraE21.setVisible(false)

      if (!this.balaoOlga.visible  && !this.olga.visible) {
        this.letraE20.setVisible(true)
        this.letraE21.setVisible(true)
        this.letraE20Interval20 = setInterval(() => {
        this.letraE21.visible = !this.letraE21.visible;
        }, 500);
      }
    ...
    // Adicionando o botão
    this.button = this.add.image(643, 405, "mecanicaDois").setScale(0.15);
    this.physics.world.enable([this.player, this.button]);
    this.button.setSize(700, 350);
    ...
    this.button.on("pointerdown", () => {
      ...
      if (this.playerOverlappingButton) {
        // Se o botão estiver habilitado pode ser apertado
        this.backgroundSong.stop();
        this.scene.start("MainBrowser");
        this.scene.stop("CenaDois");
      }
    });

    this.input.keyboard.on('keydown-E', () => {
      // Verifica se o botão está habilitado para ser acionado
      if (this.playerOverlappingButton && !this.olga.visible) {
        // Se o botão estiver habilitado pode ser apertado
        this.backgroundSong.stop();
        this.scene.start("MainBrowser");
        this.scene.stop("CenaDois");
    ...
    this.hudE.on('pointerdown', () => {
      if (this.playerOverlappingOlga) {
        ...
        if (this.dialogoAtual > this.dialogos.length) {
          ...
          this.letraE4.setVisible(false)
          this.letraE5.setVisible(false)
          clearInterval(this.letraE4Interval4); // Reinicia o diálogo para a próxima vez
        }
      }
    });
    ...
    this.movNPCazul = 0;
    this.movNPCmoreno = 0;
     // Crie um objeto para cobrir a tela durante o fade
     this.blackScreen = this.add.image(800, 420, 'selecao').setScale(0.4);
     ...
     this.closeButton = this.add.image(940, 410, 'closeButton').setScale(0.15).setInteractive(); // Adapte as coordenadas e a imagem do botão conforme necessário
     this.closeButton.setDepth(7);
...
// Adicione essa linha para registrar a cena no jogo
  ...
  update() {
   ...
   if (this.joyStick.right && !this.balaoOlga.visible) {
  ...
  if (this.joyStick.left && !this.balaoOlga.visible) {
    this.player.body.setVelocityX(-150);
  ...
  if (this.joyStick.up && !this.balaoOlga.visible) {
  ...
  if (this.joyStick.down && !this.balaoOlga.visible) {
    this.player.body.setVelocityY(150);
  ...
  if (!this.joyStick.right && !this.joyStick.left && !this.joyStick.up && !this.joyStick.down && !this.balaoOlga.visible && !this.cursors.left.isDown && this.cursors.right.isDown && this.cursors.up.isDown && this.cursors.down.isDown) {
  ...
  if (this.cursors.left.isDown && !this.balaoOlga.visible && !this.blackScreen.visible) {
    this.player.body.setVelocityX(-150);
  ...
  if (this.cursors.right.isDown && !this.balaoOlga.visible && !this.blackScreen.visible) {
    this.player.body.setVelocityX(150);
  ...
  if (this.cursors.up.isDown && !this.balaoOlga.visible && !this.blackScreen.visible) {
    this.player.body.setVelocityY(-150);
    ...
    this.player.anims.play("player_up", true);
  } else if (this.cursors.down.isDown && !this.balaoOlga.visible && !this.blackScreen.visible) {
    this.player.body.setVelocityY(150);
  ...
  if (!this.cursors.up.isDown && !this.cursors.down.isDown && !this.cursors.left.isDown && !this.cursors.right.isDown && !this.balaoOlga.visible && !this.joyStick.right && !this.joyStick.left && !this.joyStick.up && !this.joyStick.down) {
    ...
    if (Phaser.Geom.Intersects.RectangleToRectangle(this.player.getBounds(), this.olga.getBounds())) {
      this.playerOverlappingOlga = true;
    } else {
      this.playerOverlappingOlga = false;  
    }

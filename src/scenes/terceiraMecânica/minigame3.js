...
//Minigame herdando caracteristicas da classe mãe
class Minigame extends Phaser.Scene {
  constructor() {
    super({ key: "Minigame" });
  }

  //carregando src/assets que serão usados duranto o jogo
  preload() {
  ...
  create() {
    ...
    candidato1 = this.add.image(gameState.larguraJogo / 8, gameState.alturaJogo / 2, "cand1").setScale(1);
    ...
    candidato2 = this.add.image(gameState.larguraJogo / 2.8, gameState.alturaJogo / 2, "cand2");
    candidato2.setInteractive();
    
    candidato2.on("pointerdown", () => {
      this.selectSound.play();
      this.backgroundSong.stop();
      this.scene.start("Cand2");
    });
    ...
    candidato3 = this.add.image(gameState.larguraJogo / 1.55, gameState.alturaJogo / 2, "cand3");
    candidato3.setInteractive();

    candidato3.on("pointerdown", () => {
      this.selectSound.play();
      this.backgroundSong.stop();
      this.scene.start("Cand3");
    });
    ...
    candidato4 = this.add.image(gameState.larguraJogo / 1.15, gameState.alturaJogo / 2, "cand4");
    candidato4.setInteractive();

    candidato4.on("pointerdown", () => {
      this.selectSound.play();
      this.backgroundSong.stop();
      this.scene.start("Cand4");
    });
